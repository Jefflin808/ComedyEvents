...
namespace ComedyEvents.Controllers
{
    ...
    public class ComediansController : ControllerBase
    {
        ...
        [HttpPut("{comedianId}")]
        public async Task<ActionResult> Put(int comedianId, ComedianDto dto)
        {
            try
            {
        ...
        public async Task<ActionResult<ComedianDto>> Delete(int comedianId)
        {
            try
            {
                var oldComedian = await _eventRepository.GetComedian(comedianId);
                if (oldComedian == null)
                    return NotFound($"Could not find event with id {comedianId}");
